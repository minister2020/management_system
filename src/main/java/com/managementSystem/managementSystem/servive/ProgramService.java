...
import java.util.List;
    ...
    private UserRepository userRepository;
        ...
        public Program addProgram(Program program) {
            ...
            // Determine eligible users based on program mode
            List<User> eligibleUsers = userRepository.findAll().stream()
                    .filter(user -> {
                        if (program.getProgramIsFor() == ProgramMode.GENERAL) return true;
                        if (program.getProgramIsFor() == ProgramMode.MALE_ONLY && user.getGender() == Gender.MALE) return true;
                        if (program.getProgramIsFor() == ProgramMode.FEMALE_ONLY && user.getGender() == Gender.FEMALE) return true;
                        return false;
                    })
                    .toList();
            ...
            for (User user : eligibleUsers) {
                ...
                if (phone != null && !phone.isBlank()) {
                    ...
                    System.err.println("⚠️ No phone number for user: " + user);
                }
            }
    ...
    public List<Program> getAllPrograms() {
        ...
        public Program updateProgram(Long id, Program updatedProgram) {
            ...
            program.setProgramIsFor(updatedProgram.getProgramIsFor()); // ✅ Set the mode
            ...
            userRepository.findAll().forEach(user -> {
                ...
                if (phone != null && !phone.isBlank()) {
                    ...
                    System.err.println("⚠️ No phone number for member: " + user);
                }
            });
